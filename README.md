...
기존에는 문서 조회 로직이 모놀리식 서비스 내부에서 실행됐습니다. 문서 조회 부하가 증가하면서 일반 업무 API도 동일한 Thread·DB Connection·JVM 메모리를 함께 사용했고, 모놀리식 서비스의 처리 자원을 점유하는 병목이 발생했습니다.
...
문서 관리 기능은 Strangler Pattern을 적용해 문서 관리 MSA로 점진적으로 분리했습니다.

| 데이터/기능 | 담당 서비스 |
|:---|:---|
| 원본 업무 데이터 | 모놀리식 서비스 |
| 문서 목록·미리보기 데이터 | 문서 관리 MSA |
| 문서 조회·검색 API | 문서 관리 MSA |

모놀리식 서비스는 원본 데이터의 소유권을 유지하고, 문서 관리 MSA는 조회에 최적화된 문서 데이터를 관리하도록 역할을 분리했습니다.
...
- 문서 목록·미리보기 갱신을 비동기 처리해 모놀리식 서비스의 처리 부담 감소
- 데이터 변경 이벤트를 기준으로 문서 관리 MSA의 조회용 데이터를 최신화
